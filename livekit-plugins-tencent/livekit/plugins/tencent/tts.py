...
def _build_signature(secret_key: str, sign_str: str) -> str:
    digest = hmac.new(
...
def build_signed_ws_url(opts: _TencentFlowingOptions) -> tuple[str, str]:
    ...
    signature = _build_signature(opts.secret_key, sign_str)
...
class TTS(tts.TTS):
    """Tencent Cloud 流式文本语音合成（WebSocket `TextToStreamAudioWSv2`）。"""
